...
!  Complete the calculation for the remaining sublattice
        ...
        do k=2,itdim
           if(i.EQ.(k-1))then
            flag(i)=1
            call udagextract(usm1,jj,l2)
            call uextract(usm2,4,l2)
            usm1=matmul(usm1,usm2)
            l=neib(jj,l2)
            call uextract(usm4,4,l)
            l1=neib(4,l2)
            call udagextract(usm5,jj,l1)
...
!           write(*,*) l2
        enddo
         ...
         TNP(:,:,i1,jj,1,i)=TNP(:,:,i1,jj,1,i)+usm1(:,:)
!
         enddo
         ll2=l2
!
...
!           write(*,*) l2
        enddo
         ...
         TNP(:,:,i1,jj,j/itdim+1,i)=TNP(:,:,i1,jj,j/itdim+1,i)+usm3(:,:)
